...
import { Award, CheckCircle2, Coins, Copy, Loader2, ScrollText, Wallet } from 'lucide-react'
...
export default function CertificadosPage() {
  ...
  const handleMintCertificate = async () => {
    ...
    try {
      const response = await fetch('/api/certificados/mint', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ address: connectedAddress }),
      })
    ...
    <div className="min-h-screen bg-background px-4 py-10 md:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <GlassCard className="space-y-6 p-6 md:p-8">
          ...
          <div className="rounded-xl border border-white/10 p-4">
            <p className="mb-3 text-sm text-muted-foreground">Solicitar gas fee</p>
            <CTAButton
          ...
          <div className="rounded-xl border border-white/10 p-4">
            ...
            <CTAButton
              ...
              disabled={!authenticated || !connectedAddress || isMinting}
            >
              {isMinting ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
            ...
            {mintStatus?.success && (
              <div className="mt-4 rounded-lg border border-green-500/30 bg-green-500/10 p-3 text-sm">
                <div className="mb-2 flex items-center gap-2 text-green-400">
                  <CheckCircle2 className="h-4 w-4" />
                  <span>Certificado minteado correctamente</span>
                </div>
                <p className="text-muted-foreground">Session ID: {mintStatus.sessionId}</p>
                <p className="mt-1 break-all font-mono text-xs">{mintStatus.txHash}</p>
              </div>
            )}
